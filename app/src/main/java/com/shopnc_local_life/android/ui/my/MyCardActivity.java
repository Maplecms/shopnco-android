...
import com.shopnc_local_life.android.Adapter.MyCradListViewAdapter;
 ...
 *
 */
public class MyCardActivity extends Activity implements OnScrollListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		myApp = (MyApp) MyCardActivity.this.getApplication();
		...
		adapter=new MyCradListViewAdapter(MyCardActivity.this);
		datas =new ArrayList<MyCardList>();
	...
	public void info_page_list(int pangeno){
		if(myApp.getMember_id() == null || myApp.getMember_id().equals("") || myApp.getMember_id().equals("null")
    		...
    		Toast.makeText(MyCardActivity.this, "您还没有登陆，请先登陆", Toast.LENGTH_SHORT).show();
    		listview.removeFooterView(moreView);
    		listview.onRefreshComplete();
    		return ;
    	}
		
		String url=Constants.URL_MEMBER_CARD+"&member_id="+myApp.getMember_id()+"&sign="+myApp.getMember_key()+"&pagenumber="+pangeno+"&pagesize="+Constants.PARAM_PAGESIZE;
		RemoteDataHandler.asyncGet(url, new Callback() {
			...
			public void dataLoaded(ResponseData data) {
				...
				if(data.getCode() == HttpStatus.SC_OK){
					...
					ArrayList<MyCardList> list = MyCardList.newInstanceList(json);
					datas.addAll(list);
					adapter.setDatas(datas);
					adapter.notifyDataSetChanged();
				}else{
					listview.removeFooterView(moreView);
					Toast.makeText(MyCardActivity.this, "加载数据失败，请稍后重试", Toast.LENGTH_SHORT).show();
				}
			}
		});
	}
